...
def management_fee_group(result: dict[str, Any],
                         years: list[int]) -> dict[str, Any] | None:
    ...
    So it is taken straight from the projection the engine already built
    rather than recomputed from the percentage. There is no second
    calculation here, only a second place the same number is displayed.

    Returned as its own category, listed last and labelled with its basis,
    so it reads as the assumption it is rather than as a GL account that
    someone could go looking for in the accounting system. A scenario with
    no fee gets no row at all -- not a row of zeros -- so its statement is
    laid out exactly as it was before the fee existed.
    """
    rows = result.get("projection", {}).get("years") or []
    # .get() rather than [] so this module stays correct against an engine
    # build that predates the management fee entirely.
    amounts = [float((rows[i].get("management_fee") if i < len(rows) else 0.0) or 0.0)
               for i in range(len(years))]
    if not any(amounts):
        return None
    return {
        "category": MANAGEMENT_FEE_CATEGORY,
        "is_assumption": True,
        "line_count": 1,
        "lines": [{
            ...
            "gl_code": "",
            "growth_pct": None,
            # It has no growth rate of its own: being a percentage of
            # income, it already grows with income.
            "effective_growth_pct": None,
            "amounts": amounts,
        }],
        "subtotals": list(amounts),
    }
...
def build_pnl(scenario: dict[str, Any], unit_lines: list[dict[str, Any]],
    ...
    # line, so it has to join the statement here or the columns will not
    # sum to the engine's own totals -- and reconcile() would refuse to
    # render at all. Appended last: it is an assumption, not a GL account.
    fee_group = management_fee_group(result, years)
    if fee_group:
        expenses.append(fee_group)
