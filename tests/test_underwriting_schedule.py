...
class TestAbsentScheduleIsBitIdentical(unittest.TestCase):
    ...
    def test_none_and_empty_schedule_agree(self):
        a = um.analyze_scenario(self.scenario, self.units, self.expenses, assumption_years=None)
        b = um.analyze_scenario(self.scenario, self.units, self.expenses, assumption_years=[])
    ...
    def test_a_schedule_that_only_restates_the_flat_rates_changes_nothing(self):
        ...
        scheduled = um.analyze_scenario(self.scenario, self.units, self.expenses, assumption_years=rows)
        flat = flat_projection(self.scenario, self.units, self.expenses)
...
class TestDealAnalyzerUntouched(unittest.TestCase):
    ...
    def test_engine_takes_no_schedule_argument(self):
        """The engine's positional contract is unchanged and it has gained
        no per-year parameter.

        It does now carry a keyword-only `debt` override, added by the
        multi-loan work -- that is financing, not per-year assumptions, and
        this phase must not add anything alongside it. Asserted as "no
        schedule parameter exists" rather than by freezing the whole
        parameter list, so the check keeps testing this phase's claim
        instead of failing whenever some unrelated feature is added."""
        import inspect
        params = inspect.signature(dam.analyze_noi_series).parameters
        positional = [n for n, p in params.items()
                      if p.kind is p.POSITIONAL_OR_KEYWORD]
        self.assertEqual(positional, ["inputs", "noi_series", "noi_exit"])
        for name in params:
            self.assertNotIn("schedule", name)
            self.assertNotIn("assumption", name)
            self.assertNotIn("vacancy", name)
...
class TestPerYearEgiHandCalculated(unittest.TestCase):
    ...
    def test_rising_vacancy_lowers_noi(self):
        ...
        scheduled = um.analyze_scenario(self.scenario, self.units,
                                        self.expenses, assumption_years=vacancy_only)
    ...
    def test_rent_growth_alone_raises_noi(self):
        ...
        flat = um.analyze_scenario(self.scenario, self.units, self.expenses)
        scheduled = um.analyze_scenario(self.scenario, self.units,
                                        self.expenses, assumption_years=growth_only)
        self.assertEqual(scheduled["projection"]["noi_series"][0],
                         flat["projection"]["noi_series"][0])
    ...
    def test_has_schedule_is_true(self):
        r = um.analyze_scenario(self.scenario, self.units, self.expenses, assumption_years=self.rows)
