...
class TestManagementFeeLine(unittest.TestCase):
    """The management fee is subtracted to reach NOI but is not an expense
    line, so the statement has to carry it or the reconciliation refuses
    to render. Zero fee must leave the statement exactly as it was."""

    def setUp(self):
        self.scenario, self.units, self.expenses = load_fixture()

    def _pnl(self, **overrides):
        s = dict(self.scenario, **overrides)
        result = um.analyze_scenario(s, self.units, self.expenses)
        return pnl_mod.build_pnl(s, self.units, self.expenses, result), result

    def _categories(self, pnl):
        return [g["category"] for g in pnl["expenses"]]

    def test_no_fee_means_no_row(self):
        pnl, _ = self._pnl()
        self.assertNotIn(pnl_mod.MANAGEMENT_FEE_CATEGORY, self._categories(pnl))

    def test_zero_fee_means_no_row(self):
        pnl, _ = self._pnl(management_fee_pct=0.0)
        self.assertNotIn(pnl_mod.MANAGEMENT_FEE_CATEGORY, self._categories(pnl))

    def test_zero_fee_totals_match_the_confirmed_baseline(self):
        pnl, _ = self._pnl(management_fee_pct=0.0)
        self.assertAlmostEqual(pnl["expense_totals"][0], EAGLE_ROCK["opex_year1"], places=2)
        self.assertAlmostEqual(pnl["noi"][0], EAGLE_ROCK["noi_year1"], places=2)

    def test_fee_appears_as_its_own_category(self):
        pnl, _ = self._pnl(management_fee_pct=3.0)
        self.assertIn(pnl_mod.MANAGEMENT_FEE_CATEGORY, self._categories(pnl))

    def test_fee_row_is_last(self):
        """It is an assumption, not a GL account, so it sits after the
        accounts rather than alphabetically among them."""
        pnl, _ = self._pnl(management_fee_pct=3.0)
        self.assertEqual(self._categories(pnl)[-1], pnl_mod.MANAGEMENT_FEE_CATEGORY)

    def test_fee_row_is_marked_as_an_assumption(self):
        pnl, _ = self._pnl(management_fee_pct=3.0)
        group = pnl["expenses"][-1]
        self.assertTrue(group["is_assumption"])

    def test_fee_amounts_come_from_the_engine_not_recomputed(self):
        pnl, result = self._pnl(management_fee_pct=3.0)
        group = pnl["expenses"][-1]
        engine = [y["management_fee"] for y in result["projection"]["years"]]
        self.assertEqual(group["subtotals"], engine)

    def test_reconciliation_ties_with_a_fee(self):
        pnl, result = self._pnl(management_fee_pct=3.0)
        self.assertTrue(all(c["passed"] for c in pnl["reconciliation"]))
        self.assertAlmostEqual(pnl["expense_totals"][0],
                               result["operating_expenses_year1"], places=6)

    def test_every_year_ties_with_a_fee(self):
        pnl, result = self._pnl(management_fee_pct=3.0)
        for i, py in enumerate(result["projection"]["years"]):
            self.assertAlmostEqual(pnl["expense_totals"][i], py["expenses"], places=6)
            self.assertAlmostEqual(pnl["noi"][i], py["noi"], places=6)

    def test_capital_transaction_fee_does_not_reach_the_statement(self):
        """It is an exit cost, not an operating expense -- it must not
        appear in a P&L year."""
        with_ctf, result = self._pnl(capital_transaction_fee_pct=1.0)
        self.assertNotIn(pnl_mod.MANAGEMENT_FEE_CATEGORY, self._categories(with_ctf))
        self.assertAlmostEqual(with_ctf["expense_totals"][0],
                               EAGLE_ROCK["opex_year1"], places=2)
        self.assertGreater(result["returns"]["capital_transaction_fee"], 0)

    def test_gate_still_fires_if_the_fee_row_were_dropped(self):
        """The fix must not have been to loosen the check."""
        pnl, result = self._pnl(management_fee_pct=3.0)
        pnl["expenses"] = [g for g in pnl["expenses"]
                           if g["category"] != pnl_mod.MANAGEMENT_FEE_CATEGORY]
        pnl["expense_totals"] = [sum(g["subtotals"][i] for g in pnl["expenses"])
                                 for i in range(len(pnl["years"]))]
        with self.assertRaises(pnl_mod.PnLReconciliationError):
            pnl_mod.reconcile(pnl, result)

    def test_fee_row_appears_in_the_exports(self):
        pnl, _ = self._pnl(management_fee_pct=3.0)
        labels = [r["label"] for r in pnl_export.flatten_rows(pnl)]
